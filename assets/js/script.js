...
document.addEventListener('DOMContentLoaded', () => {
  ...
  try {
      ...
      document.querySelector('.dice-section').addEventListener('touchend', (event) => {
      ...
      ['green', 'red', 'blue', 'yellow'].forEach(color => {
          document.getElementById(`${color}Threshold`).addEventListener('change', (event) => {
              ...
              if (characterName && characterName !== 'new') {
                  ...
                  characters = characters.map(char =>
                      char.name === characterName ? { name: characterName, thresholds: { ...thresholds } } : char
                  );
  ...
  function displayResults(results) {
      ['green', 'red', 'blue', 'yellow'].forEach(color => {
          const resultContainer = document.querySelector(`#${color}Results .results`);
...
function loadSelectedCharacter() {
  ...
  if (characterName === 'new') {
      ...
      // Thresholds are set via dropdowns, no need to reset here
...
function updateThresholds() {
  ...
  if (characterName && characterName !== 'new') {
      ...
      let characters = JSON.parse(localStorage.getItem('characters')) || [];
      characters = characters.map(char => 
          char.name === characterName ? { name: characterName, thresholds: updatedThresholds } : char
      );
      localStorage.setItem('characters', JSON.stringify(characters));
